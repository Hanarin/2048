...
type BlockProps = Omit<BlockType, "id">;

const COLOR_MAP: Record<number, { background: string; color: string }> = {
...
const FALLBACK = { background: "#E6A817", color: "#333" };

const Block = ({ value, row, col, isNew, merged, toRemove }: BlockProps) => {
  const wrapperStyle = {
    "--cell-size": "94px",
    "--gap": "8px",
    "--row": row,
  ...
  const { background, color } = COLOR_MAP[value] ?? FALLBACK;
